...
def safe_exponentiate(base: Union[int, float],
    ...
    if base == 0 and exponent == 0:
        return 1  # Handle 0^0 case
    return base ** exponent

def dice_posteror(sample_draw: List[int], 
                   die_type_priors: List[float],
                   dice: List[Die]) -> List[float]:
    """
    Calculates the posterior probability of each die type given the number of 
    times each face appeared in the draw (sample_draw), the prior probabilities of 
    each die type, and the face probabilities for each die type.

    :param sample_draw: A list of integers representing how many times each face 
                        was rolled in the draw. The length of sample_draw should 
                        match the number of faces on the dice.
    :param die_type_priors: A list of prior probabilities for selecting each die.
                            This should sum to 1.
    :param dice: A list of Die objects representing the dice in the bag. Each Die 
                 object contains the probabilities for rolling each face.
    
    :return: A list of posterior probabilities for each die type.
    """
    # avoid errors
    assert np.isclose(sum(die_type_priors), 1.0), "Priors must sum to 1"
...
def e_step(experiment_data: List[NDArray[np.int_]],
    ...
    for draw in experiment_data:
        posteriors = dice_posteror(draw, bag_of_dice.die_priors, bag_of_dice.dice) # compute the posterior 
        for i, die in enumerate(bag_of_dice.dice):
            expected_counts[i, :len(die)] += posteriors[i] * draw
...
def m_step(expected_counts_by_die: NDArray[np.float_]):
    ...
    updated_priors = np.sum(expected_counts_by_die, axis=1)
